...
- **UI work is bound by `.claude/DESIGN_SYSTEM.md`** — tokens, typography, spacing, motion, and component rules defined there apply for the lifetime of the project. Never use raw hex, raw px (outside token files), or primitive color steps in component CSS. If a need isn't covered, extend the design system doc first, then build.
